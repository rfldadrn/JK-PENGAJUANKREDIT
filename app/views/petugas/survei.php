<?php
ob_start();
?>

<!-- List Survei -->
<div class="card border-0 shadow-sm">
    <div class="card-header bg-white">
        <h5 class="mb-0"><i class="bi bi-geo-alt me-2"></i>Daftar Pengajuan untuk Survei Lapangan</h5>
    </div>
    <div class="card-body p-0">
        <?php if (empty($pengajuanList)): ?>
            <div class="text-center py-5">
                <i class="bi bi-geo-alt display-4 text-muted"></i>
                <p class="text-muted mt-3 mb-0">Tidak ada pengajuan untuk survei</p>
            </div>
        <?php else: ?>
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    <thead class="table-light">
                        <tr>
                            <th>No. Pengajuan</th>
                            <th>Nasabah</th>
                            <th>Alamat Survei</th>
                            <th>Jenis Kredit</th>
                            <th>Jumlah</th>
                            <th>Status</th>
                            <th>Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($pengajuanList as $item): ?>
                        <tr>
                            <td><strong><?= $item['no_pengajuan'] ?></strong></td>
                            <td><?= htmlspecialchars($item['nama_lengkap']) ?></td>
                            <td><?= htmlspecialchars(substr($item['alamat_ktp'], 0, 50)) ?>...</td>
                            <td><?= $item['nama_kredit'] ?></td>
                            <td>Rp <?= number_format($item['jumlah_pinjaman'], 0, ',', '.') ?></td>
                            <td>
                                <span class="badge bg-primary">Survei</span>
                            </td>
                            <td>
                                <a href="<?= BASE_URL ?>petugas/survei/<?= $item['id_pengajuan'] ?>" class="btn btn-sm btn-warning">
                                    <i class="bi bi-geo-alt"></i> Input Survei
                                </a>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
            </div>
        <?php endif; ?>
    </div>
</div>

<?php
$content = ob_get_clean();

$pageTitle = 'Survei Lapangan';
$pageSubtitle = 'Input hasil survei lapangan';

require_once BASE_PATH . 'app/views/layouts/dashboard.php';
?>
